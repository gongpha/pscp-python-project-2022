...
class PYPORT(Node):
    ...
    def settings_save(self):
        ...
        config.set_value("settings", "master_volume",
                         settings.get("master_volume", 1.0))
        config.set_value("settings", "music_volume",
                         settings.get("music_volume", 1.0))
        config.set_value("settings", "fullscreen",
    ...
    def settings_apply(self):
        settings = self.get("settings")
        AudioServer.set_bus_volume_db(AudioServer.get_bus_index(
            "Master"), linear2db(settings.get("master_volume", 1.0)))
        AudioServer.set_bus_volume_db(AudioServer.get_bus_index(
            "Music"), linear2db(settings.get("music_volume", 1.0)))
