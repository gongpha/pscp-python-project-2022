...
from godot import exposed, Control, InputEventKey, KEY_UP, KEY_DOWN
import ctypes
...
class PSCPProjectGrouppeConsole(InteractiveConsole):
...
class console(Control):
    ...
    def _ready(self):
        ...
        self.output_box = self.get_node("vbox/rich")
        self.run_button = self.get_node("vbox/hbox/submit")
        self.run_button.connect("pressed", self, "execute")
        self.clear_button = self.get_node("vbox/hbox2/clear")
        self.clear_button.connect("pressed", self, "clear")
        self.interrupt_button = self.get_node("vbox/hbox2/interrupt")
        self.interrupt_button.connect("pressed", self, "send_keyboard_interrupt")
        self.input_box = self.get_node("vbox/hbox/input")
        self.input_box.connect("text_entered", self, "execute")
        self.input_box.connect("gui_input", self, "_input_gi")
        self.back_mm = self.get_node("vbox/hbox2/back")
        self.back_mm.connect("pressed", self, "back")
        ...
        if not console.__STREAMS_CAPTURE_INSTALLED:
            console.__STREAMS_CAPTURE_INSTALLED = True
        ...
        self.interpreter = PSCPProjectGrouppeConsole(
    ...
    def _exit_tree(self):
        ...
        if self.__plugin_instantiated:
            console.__STREAMS_CAPTURE_INSTALLED = False
